...
def filter_database():
    ...
    for record in records:
        ...
        for place in places:
        	originPlace = {"geometry":{"type":"Point"}}
		originPlace["geometry"]["coordinates"] = [record.lat, record.lng]
		output["originP"].append(originPlace)
                connectedPlace = {"abc":{"type":"Point"}}
		connectedPlace["abc"]["coordinates"] = [place.lat, place.lng]
		output["connectedP"].append(connectedPlace)
    
    #print output['originP'][0]
    #print output['connectedP'][0]

    #show connections between lists
    #for i in range(len(originPlaces)):
    #	print originPlaces[i] 
